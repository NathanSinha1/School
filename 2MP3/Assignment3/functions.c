...
void ReadMMtoCSR(const char *filename, CSRMatrix *matrix) {
    ...
    for (int i = 0; i < matrix->num_non_zeros - 1; i++) { // bubble sort!!!! (slows down code signifcantly(sad))
        for (int j = 0; j < matrix->num_non_zeros - 1; j++) {
            if (matrix->row_ptr[j + 1] < matrix->row_ptr[j]) {
                int rowTemp = matrix->row_ptr[j + 1];
                int colTemp = matrix->col_ind[j + 1];
                double valTemp = matrix->csr_data[j + 1];

                matrix->row_ptr[j + 1] = matrix->row_ptr[j];
                matrix->col_ind[j + 1] = matrix->col_ind[j];
                matrix->csr_data[j + 1] = matrix->csr_data[j];

                matrix->row_ptr[j] = rowTemp;
                matrix->col_ind[j] = colTemp;
                matrix->csr_data[j] = valTemp;
            }
        }
    }
